...
export function useDevicesApi() {
  return {
    ...
    remove: async (deviceId: string, version: number) =>
      apiDelete(`/api/devices/${deviceId}`, { version }),
    importCsv: async (homeId: string, file: File) => {
      const formData = new FormData()
      formData.append('file', file)
      return (await apiUpload<DeviceImportSummary>(`/api/homes/${homeId}/devices/import`, formData))
        .data
    },
    suggestions: {
