...
export function useRoomsApi() {
  return {
    listForHome: async (homeId: string) =>
      (await apiGet<RoomDto[]>(`/api/homes/${homeId}/rooms`)).data,
    ...
    remove: async (roomId: string, version: number) =>
      apiDelete(`/api/rooms/${roomId}`, { version }),
